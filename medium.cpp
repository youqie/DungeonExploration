...
medium::medium(QWidget *parent,bool ismusic) : QMainWindow(parent)
{
    ismusic3=ismusic;
    ...
    connect(menuBtn,&MyPushButton::clicked,[=](){
        if(ismusic3) clickSound->play();
        menuScene = new menu(nullptr,ismusic3);
        menuScene->show();
...
void medium::paintEvent(QPaintEvent *)
{
...
//实现金币的创建和翻转
void medium::Flip(int i,int j,QSound * gameSound)
{
    ...
    connect(cards,&Cards::clicked,[=](){
        ...
        else if(count==2)//第二步
        {
            ...
            if(victoryflag==8)
            {
                if(level==5)
                {
                    winfinalScene = new winfinal(nullptr,ismusic3);
                    winfinalScene->show();
            ...
            {
                loseScene = new lose(nullptr,ismusic3);
                loseScene->show();
...
void medium::exchangecards(Cards *card1,Cards *card2)
{
    //交换两个cards对象
